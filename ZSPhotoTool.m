...
@implementation ZBSelectPhotoModel
@end
@implementation ZBPhotoAblumList : NSObject
@end

@implementation ZSPhotoTool
static ZSPhotoTool *sharePhotoTool = nil;
...
{
    NSMutableArray<ZBPhotoAblumList *> *photoAblumList = [NSMutableArray array];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if (!([collection.localizedTitle isEqualToString:@"Recently Deleted"] ||
            ...
            if (assets.count > 0) {
                ZBPhotoAblumList *ablum = [[ZBPhotoAblumList alloc] init];
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *photosBytes))completion
{
    ...
    for (int i = 0; i < photos.count; i++) {
        ZBSelectPhotoModel *model = photos[i];
